...
const CategoryToolsGrid = ({
  ...
  return (
    <section className="mb-20">
      <div className={`mb-12 pb-8 border-b-2`} style={{ borderColor: colorTheme.borderColor }}>
        <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-foreground mb-3" style={{ color: colorTheme.accentColor }}>
          {categoryName}
        </h3>
        ...
        {tools.map((tool) => {
          ...
          return (
            <Link key={tool.title} to={tool.to}>
              <div className="flex h-24 overflow-hidden bg-card shadow-lg rounded-xl hover:shadow-xl transition-all hover:scale-105" style={{ borderLeft: `4px solid ${colorTheme.accentColor}` }}>
                ...
                <div className="flex items-center justify-center px-3 flex-shrink-0">
                  <Icon height={30} width={30} stroke={colorTheme.iconColor} strokeWidth="1.5" />
                </div>
