      ...
      <div className="container mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-0 justify-center flex-1">
            ...
            {toolCategories.slice(0, 3).map((category, idx) => {
              ...
              return (
                <Link
                  key={category.name}
                  to={firstTool.to}
                  className="px-4 py-4 text-sm font-bold text-gray-900 hover:text-primary transition-colors border-b-2 border-transparent hover:border-primary whitespace-nowrap"
                >
            ...
            <div className="relative group">
              <button
                ...
                className="flex items-center gap-2 px-4 py-4 text-sm font-bold text-gray-900 hover:text-primary transition-colors border-b-2 border-transparent hover:border-primary"
              >
                CONVERT PDF
              ...
              {activeDropdown === "all" && (
                <>
                  <div className="absolute left-1/2 -translate-x-1/2 top-full mt-0 w-4 h-2 z-50 pointer-events-none">
                    ...
                    onMouseEnter={() => setActiveDropdown("all")}
                    onMouseLeave={() => setActiveDropdown(null)}
                     className="absolute left-1/2 -translate-x-[63%] top-full mt-2 bg-white border border-gray-200 shadow-lg z-50 p-6 rounded-2xl"
                    >
          ...
          <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
            <button className="hidden sm:block text-gray-900 hover:text-primary transition-colors text-sm font-bold px-3 py-2">
              Login
            </button>

            <button className="hidden sm:block bg-primary hover:bg-primary/90 text-white px-5 py-2 rounded-md text-sm font-bold transition-colors">
              Sign up
            </button>
      ...
      {mobileMenuOpen && (
        <nav className="lg:hidden border-t border-gray-200 bg-white pb-4">
          ...
          {toolCategories.slice(0, 3).map((category, idx) => {
            ...
            return (
              <Link
                key={category.name}
                to={firstTool.to}
                className="block px-4 py-3 text-gray-900 hover:bg-gray-50 transition-colors text-sm font-bold border-b border-gray-100"
              >
                {category.name.replace(" Tools", "").toUpperCase()}
              </Link>
            );
          ...
          <div className="px-4 py-3 space-y-2 border-t border-gray-100">
            <button className="w-full text-gray-900 hover:bg-gray-50 transition-colors text-sm font-bold px-3 py-2 rounded-md">
              Login
            </button>
            <button className="w-full bg-primary hover:bg-primary/90 text-white px-3 py-2 rounded-md text-sm font-bold transition-colors">
              Sign up
            </button>
          </div>
        </nav>
