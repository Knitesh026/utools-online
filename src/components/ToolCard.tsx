...
const Card = ({ title, description = "", icon: Icon, number, to, colorTheme }: CardProps) => {
  ...
  const content = (
    <div className="flex w-full h-16 overflow-hidden bg-card shadow-lg rounded-sm sm:rounded-[0.5rem] hover:shadow-xl transition-all hover:scale-105" style={{ borderLeft: `4px solid ${accentColor}` }}>
      ...
      <div className="flex items-center justify-center px-2 sm:px-2 flex-shrink-0">
        <Icon height={26} width={26} stroke={iconColor} strokeWidth="1.5" />
      </div>
