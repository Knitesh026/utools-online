...
const Card = ({ title, description = "", icon: Icon, number, to, colorTheme }: CardProps) => {
  ...
  const content = (
    <div className="flex w-full h-24 overflow-hidden bg-card shadow-lg rounded-xl hover:shadow-xl transition-all hover:scale-105" style={{ borderLeft: `4px solid ${accentColor}` }}>
      ...
      <div className="flex items-center justify-center px-3 flex-shrink-0">
        <Icon height={30} width={30} stroke={iconColor} strokeWidth="1.5" />
      </div>
      <div className="mx-2.5 overflow-hidden w-full flex flex-col justify-center">
        <p className="text-lg font-bold overflow-hidden text-ellipsis whitespace-nowrap" style={{ color: accentColor }}>
          {title}
        </p>
