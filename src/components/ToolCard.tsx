...
const Card = ({ title, description = "", icon: Icon, number, to, colorTheme }: CardProps) => {
  ...
  const content = (
    <div className="flex w-full h-14 sm:h-16 overflow-hidden bg-card shadow-lg rounded-sm sm:rounded-[0.5rem] hover:shadow-xl transition-all hover:scale-105" style={{ borderLeft: `4px solid ${accentColor}` }}>
