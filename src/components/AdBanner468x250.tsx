...
export const AdBanner468x250 = ({ className = "" }: { className?: string }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const bannerId = useMemo(() => `banner_468x250_${Date.now()}_${Math.random()}`, []);
  ...
  return (
    <div className={`w-full flex justify-center px-2 sm:px-4 py-4 ${className}`}>
      <div 
        ref={containerRef} 
        className="ad-container-468x250"
        style={{ 
          minHeight: "250px", 
          width: "100%",
          maxWidth: "468px",
...
export default AdBanner468x250;
